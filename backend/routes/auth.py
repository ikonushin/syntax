...
@router.get(
    "/user-consents",
    status_code=status.HTTP_200_OK,
    summary="Get user's active consents",
    description="Get list of all active consents for a user"
)
async def get_user_consents(
    ...
    """
    Get list of user's active consents.
    
    **Query parameters:**
    - `user_id`: User identifier (e.g., "team286-7")
    - `access_token`: JWT token
    
    **Returns:**
    - List of active consents with bank info
    """
    try:
        logger.info(f"🔍 USER_CONSENTS DEBUG: Getting consents for user {user_id}")
        
        # Query database for user's consents
        statement = select(Consent).where(
            Consent.client_id == user_id,
            Consent.status.in_(["approved", "authorized", "pending"])
        )
        consents = session.exec(statement).all()
        
        logger.info(f"🔍 USER_CONSENTS DEBUG: Found {len(consents)} consents in DB")
        ...
        logger.error(f"🔍 USER_CONSENTS DEBUG: Error: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error fetching user consents: {str(e)}"
        )
...
async def get_user_consents(
    ...
    **Returns:**
    ...
    if not user_id or not access_token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="user_id and access_token are required"
        )
    
    try:
        logger.info(f"🔍 USER_CONSENTS: Getting consents for user {user_id}")
        
        # Decode JWT to verify token
        try:
            token_data = decode_token(access_token)
            client_id = token_data.get("client_id")
        ...
        statement = select(Consent).where(
            Consent.client_id == user_id,
            Consent.status.in_(["approved", "authorized", "success", "pending", "awaitingAuthorization"])
        )
        consents = session.exec(statement).all()
