...
|------|------|--------|-------|
| **VBank** | Auto-approval | `POST /account-consents/request` + `auto_approved: true` | `{status: "success", consent_id: "..."}` |
...
```javascript
// VBank/ABank: показать успех и редирект на транзакции
...
// SBank: показать сообщение и редирект в ЛК банка
...
{
  "status": "success",
  "consent_id": "consent-93edc6dfd8a3",
  "redirect_url": null
}
```
